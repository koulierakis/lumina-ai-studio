...
11. Engine status explicitly reports that OpenHands runtime validation is still pending.
12. OpenHands proposals report `awaiting_approval`, `review_only: true`, `applied: false`, `can_apply: false`, `source_repository_unchanged: true`, `next_action: review_changes`, `ready: false`, and `runtime_validated: false` until a real runtime task succeeds.
