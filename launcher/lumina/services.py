...
def _acquire_lock(repo_root: Path) -> None:
    ...
    if path.exists():
        ...
        if existing and pid_exists(existing):
            raise AlreadyRunningError(f"LUMINA runtime lock is held by process {existing}. Use 'status' or 'stop' if you need to recover.")
        try:
...
def _ensure_dependencies(cfg: dict[str, Any]) -> None:
    ...
    if not ollama["ok"] and cfg.get("automatic_ollama_startup"):
        raise DependencyMissingError("Ollama is not installed or not on PATH. Install Ollama or disable automatic Ollama startup.")
...
def _start_ollama_if_needed(repo_root: Path, cfg: dict[str, Any]) -> list[str]:
    ...
    if status.get("online"):
        if not status.get("installed"):
            warnings.append(f"Ollama is online but model '{cfg['preferred_ollama_model']}' is not installed.")
        return warnings
    ...
    proc = subprocess.Popen(cmd, cwd=str(repo_root), stdout=log_handle, stderr=subprocess.STDOUT, creationflags=_creation_flags(), shell=False)
    record_service("ollama", proc.pid, cmd, repo_root=repo_root)
    state = load_state(repo_root); state["owned_ollama"] = True; save_state(state, repo_root)
    ready = wait_until(lambda: bool(check_ollama(cfg["ollama_host"], cfg["ollama_port"], cfg["preferred_ollama_model"]).get("online")), timeout=min(60, cfg["startup_timeout_seconds"]), interval=cfg["readiness_poll_interval_seconds"], label="Ollama")
    if not ready:
...
def _start_backend(repo_root: Path, cfg: dict[str, Any]) -> None:
    ...
    if check_backend(probe, port).get("ok"):
        state = load_state(repo_root); pid = (state.get("services") or {}).get("backend", {}).get("pid")
        if owns_process("backend", pid, repo_root=repo_root): return
        raise PortInUseError("backend", port)
    if port_in_use(probe, port): raise PortInUseError("backend", port)
    ...
    proc = subprocess.Popen(cmd, cwd=str(repo_root / "backend"), stdout=log_handle, stderr=subprocess.STDOUT, env=env, creationflags=_creation_flags(), shell=False)
    record_service("backend", proc.pid, cmd, repo_root=repo_root)
    if not wait_until(lambda: bool(check_backend(probe, port).get("ok")), timeout=cfg["startup_timeout_seconds"], interval=cfg["readiness_poll_interval_seconds"], label="Backend"):
...
def _start_frontend(repo_root: Path, cfg: dict[str, Any]) -> None:
    ...
    if check_frontend(probe, port).get("ok"):
        state = load_state(repo_root); pid = (state.get("services") or {}).get("frontend", {}).get("pid")
    ...
    env["PORT"] = str(port)
    env["HOST"] = host
    # Empty URL means same-origin /api. This is essential for phones: the browser
    # must not try to call its own 127.0.0.1 when LUMINA is opened remotely.
...
def _open_dashboard(repo_root: Path, cfg: dict[str, Any]) -> None:
    ...
    try:
        ...
        state.setdefault("warnings", []).append(f"Browser open failed: {exc}"); save_state(state, repo_root)


def start_all(repo_root: Path | None = None) -> dict[str, Any]:
    root = repo_root or find_repo_root(); cfg = load_config(root); cleanup_stale_pids(root)
    if is_lumina_running(root): raise AlreadyRunningError()
    _ensure_dependencies(cfg); _acquire_lock(root)
    state = load_state(root); state["started_at"] = time.time(); state["browser_opened"] = False; state["warnings"] = []; save_state(state, root)
    try:
        ...
        _release_lock(root); raise


def stop_all(repo_root: Path | None = None, *, release_lock: bool = True) -> dict[str, Any]:
    root = repo_root or find_repo_root(); cleanup_stale_pids(root); state = load_state(root)
    results = {"frontend": stop_owned_service("frontend", root), "backend": stop_owned_service("backend", root)}
    results["ollama"] = stop_owned_service("ollama", root) if state.get("owned_ollama") else {"service": "ollama", "skipped": True, "reason": "not owned by LUMINA"}
    clear_state(root)
    if release_lock: _release_lock(root)
