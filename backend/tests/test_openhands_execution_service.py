...
def test_review_summary_has_change_counts(tmp_path:Path):
    (tmp_path/"existing.txt").write_text("original\n",encoding="utf-8");s=OpenHandsExecutionService(adapter=FakeAdapter()).execute(repository_root=tmp_path,instruction="change files").public_summary();assert s["changed_files"]==2 and s["change_counts"]=={"created":1,"modified":1,"deleted":0}
