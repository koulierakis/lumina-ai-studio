...
def test_registry_always_keeps_native_engine(): assert registry(False).options()[0].available is True

def test_registry_defaults_missing_selection_to_native(): assert registry(False).validate_selection(None) == "native"
...
def test_registry_public_status_is_ui_ready():
    ...
    assert payload["default"] == "native" and payload["engines"][1]["name"] == "openhands"

def test_registry_routes_openhands_result_for_review_and_requires_approval():
    payload = registry(True).execute_for_review(engine="openhands", repository_root="repo", instruction="fix it")
    assert payload["successful"] is True and payload["engine"] == "openhands" and payload["requires_approval"] is True
