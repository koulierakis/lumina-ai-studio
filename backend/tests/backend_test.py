...
EXPECTED_AI_TOOLS = {
    ...
    "generate", "inpaint", "expand", "face_restore",
    "identity_preserve", "style_transfer", "color_correct", "hdr",
    "skin_cleanup", "portrait_enhance", "watermark_remove_legal",
    "perspective_correct",
}
...
class TestAiEditFoundation:
    ...
    def test_ai_tools_catalog_shape(self, api_client, auth_headers):
        ...
        assert len(tools) == len(EXPECTED_AI_TOOLS), (
            f"expected {len(EXPECTED_AI_TOOLS)} tools, got {len(tools)}: {keys}"
        )
