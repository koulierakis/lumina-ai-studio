...
class OpenHandsExecutionService:
    def __init__(self, adapter: OpenHandsAdapter | None = None, workspace_service: OpenHandsWorkspaceService | None = None) -> None:
        self.adapter = adapter or OpenHandsAdapter(); self.workspace_service = workspace_service or OpenHandsWorkspaceService()
    @staticmethod
    ...
    def _text_diff(path: str, before: bytes | None, after: bytes | None) -> str:
        try:
            old=(before or b"").decode("utf-8").splitlines(keepends=True); new=(after or b"").decode("utf-8").splitlines(keepends=True)
        ...
        return diff if len(diff)<=MAX_REVIEW_DIFF_CHARACTERS else diff[:MAX_REVIEW_DIFF_CHARACTERS]+"\n[diff truncated for safe review]\n"
    def execute(self, *, repository_root: str | Path, instruction: str) -> OpenHandsExecutionResult:
        ...
        try:
            ...
            after=self._files(workspace.workspace_root); changes=[]
