...
class OpenHandsExecutionService:
    ...
    @staticmethod
    def _files(root:Path)->dict[str,bytes]:return{p.relative_to(root).as_posix():p.read_bytes()for p in root.rglob("*")if p.is_file()and p.name!=".lumina_openhands_sandbox"}
    @staticmethod
    def _text_diff(path:str,before:bytes|None,after:bytes|None)->str:
        try:old=(before or b"").decode("utf-8").splitlines(keepends=True);new=(after or b"").decode("utf-8").splitlines(keepends=True)
        except UnicodeDecodeError:return"[binary file changed]"
        diff="".join(difflib.unified_diff(old,new,fromfile=f"a/{path}",tofile=f"b/{path}"));return diff if len(diff)<=MAX_REVIEW_DIFF_CHARACTERS else diff[:MAX_REVIEW_DIFF_CHARACTERS]+"\n[diff truncated for safe review]\n"
