...
OpenHands must never receive the live LUMINA repository as its autonomous
workspace. This service creates a temporary copy, excludes volatile/heavy
runtime directories, and provides a deterministic cleanup boundary.
"""
    ...
    def prepare(self, repository_root: str | os.PathLike[str]) -> DisposableOpenHandsWorkspace:
        ...
        try:
            shutil.copytree(
                source,
                destination,
                ignore=shutil.ignore_patterns(*sorted(self.ignored_names)),
                symlinks=False,
            )
        except Exception as exc:
            shutil.rmtree(temp_parent, ignore_errors=True)
            raise OpenHandsWorkspaceError(
                f"Could not create disposable OpenHands workspace: {exc}"
            ) from exc

        marker = destination / ".lumina_openhands_sandbox"
        marker.write_text(
