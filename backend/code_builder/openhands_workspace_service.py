...
_DEFAULT_IGNORES:Final[frozenset[str]]=frozenset({".git",".lumina-runtime",".pytest_cache","__pycache__","node_modules","dist","build",".env",".env.local",".env.production","venv",".venv"})
...
class DisposableOpenHandsWorkspace:
    source_root:Path;workspace_root:Path
...
class OpenHandsWorkspaceService:
    ...
    def prepare(self,repository_root:str|os.PathLike[str])->DisposableOpenHandsWorkspace:
        ...
        except Exception as exc:shutil.rmtree(temp_parent,ignore_errors=True);raise OpenHandsWorkspaceError(f"Could not create disposable OpenHands workspace: {exc}")from exc
        (destination/".lumina_openhands_sandbox").write_text("Disposable LUMINA workspace. Autonomous edits are allowed here only.\n",encoding="utf-8");return DisposableOpenHandsWorkspace(source,destination)
