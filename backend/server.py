...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    payload["validation_errors"] = jsonable_encoder(exc.errors(), custom_encoder={ValueError: str})
...
VOICE_STYLES = {"natural", "calm", "warm", "professional", "energetic", "storytelling", "cinematic", "documentary", "podcast", "audiobook", "radio", "commercial", "emotional", "corporate", "motivational", "luxury"}
...
async def create_voice_job(request: Request, background: BackgroundTasks, text: str = Form(""), mode: str = Form("text-to-speech"), voice: str = Form("personal-user"), style: str = Form("podcast"), preset_id: Optional[str] = Form(None), output_format: str = Form("wav"), sample_rate: int = Form(48000), bit_depth: int = Form(24), bitrate: str = Form("192k"), loudness_lufs: float = Form(-16), title: str = Form(""), tags: str = Form(""), provider: Optional[str] = Form(None), reference_text: str = Form(""), owner: str = Depends(require_owner)) -> VoiceJob:
    ...
    if selected == "omnivoice":
        ...
        reference_candidate = (await request.form()).get("reference_audio")
        reference_audio = reference_candidate if hasattr(reference_candidate, "read") else None
