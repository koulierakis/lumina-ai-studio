...
def _register_builtin_alias(name: str, face_name: str) -> None:
    """Register a non-Unicode built-in fallback so export never crashes."""
...
def ensure_pdf_font_aliases() -> dict[str, str | bool | None]:
    """Ensure the font names expected by the legacy export service always exist."""
    ...
    if PDF_FONT_NAME not in registered:
        if regular_path is not None:
            ...
            _register_builtin_alias(PDF_FONT_NAME, "Helvetica")
    ...
    return {
        "regular_registered": PDF_FONT_NAME in pdfmetrics.getRegisteredFontNames(),
        "bold_registered": PDF_FONT_BOLD_NAME in pdfmetrics.getRegisteredFontNames(),
        "regular_path": str(regular_path) if regular_path else None,
        "bold_path": str(bold_path) if bold_path else None,
        "unicode_font_available": regular_path is not None,
    }
