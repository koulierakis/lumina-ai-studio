...
export default function DocumentStudio() {
  ...
  async function saveEditor(autosave = false) {
    ...
    try {
      if (autosave && htmlAtSave === lastSavedHtmlRef.current && !layoutDirty) { setStatusText('Ready'); return selected; }
      ...
      if (saveSequence !== saveSequenceRef.current) return updated; selectDocument(updated, false); await refreshDocuments(); if (!autosave) toast.success('Document saved.'); return updated;
    } catch (error) { toast.error(error.message || 'Save failed.'); return null; }
  ...
  async function applyPreset(presetId) {
    ...
    try {
      const contentDirty = editorHtml !== lastSavedHtmlRef.current || layoutDirty;
      if (contentDirty) {
        const saved = await saveEditor(true);
        if (!saved) return;
      }
      const updated = await documentApi.redesign(selected.id, presetId); selectDocument(updated, false); await refreshDocuments(); toast.success(`${PRESET_BUTTONS.find((p) => p.id === presetId)?.label || presetId} applied — text preserved.`);
    }
