...
function sameLayout(left, right) {
  return JSON.stringify(normalizePageLayout(left)) === JSON.stringify(normalizePageLayout(right));
}
...
export default function DocumentStudio() {
  ...
  const currentEditorHtmlRef = useRef('');
  const currentPageLayoutRef = useRef(DEFAULT_PAGE_LAYOUT);
  ...
  useEffect(() => { currentEditorHtmlRef.current = editorHtml; }, [editorHtml]);
  useEffect(() => { currentPageLayoutRef.current = normalizedPageLayout; }, [normalizedPageLayout]);
  ...
  useEffect(() => {
    ...
    if (loading || busy || saving || reviewMode === 'viewing' || (!contentDirty && !layoutDirty)) return undefined;
    autosaveTimerRef.current = window.setTimeout(() => saveEditor(true), AUTOSAVE_DELAY_MS);
    return () => window.clearTimeout(autosaveTimerRef.current);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [busy, editorHtml, layoutDirty, loading, reviewMode, saving, selected?.id]);
  ...
  function updatePageLayout(mutator) {
    setPageLayout((current) => {
      const next = normalizePageLayout(typeof mutator === 'function' ? mutator(current) : mutator);
      currentPageLayoutRef.current = next;
      return next;
    });
    setLayoutDirty(true);
  }
  function onEditorChange(nextHtml) {
    if (reviewMode !== 'viewing' && nextHtml !== currentEditorHtmlRef.current) {
  ...
  async function saveEditor(autosave = false) {
    ...
    const layoutAtSave = normalizePageLayout(currentPageLayoutRef.current);
    const layoutWasDirty = layoutDirty;
    setSaving(true);
    ...
    try {
      ...
      const updated = await documentApi.update(selectedAtSave.id, {
        ...
        metadata: { ...(selectedAtSave.metadata || {}), page_layout: layoutAtSave, export_layout: exportLayout, page_count: pageFlow.pageCount },
        expected_version: selectedAtSave.version_number,
        autosave,
        change_note: autosave ? 'Autosave' : 'Manual save',
      });
      if (saveSequence !== saveSequenceRef.current || selectedAtSave.id !== selected?.id) return;
      lastSavedHtmlRef.current = htmlAtSave;
      setSelected(updated);
      if (sameLayout(currentPageLayoutRef.current, layoutAtSave)) setLayoutDirty(false);
      await refreshDocuments();
      if (!autosave) toast.success('Document saved.');
    } catch (error) {
      if (error?.status === 409 || error?.code === 'http_409') {
        toast.error('This document changed elsewhere. Reload it before saving again.');
      } else {
  ...
  function selectDocument(document, closeLibrary = true) {
    const nextHtml = document.content_html || '';
    const nextLayout = normalizePageLayout(document.design?.pageLayout || document.metadata?.page_layout);
  ...
  const operationLocked = busy || saving;

  return (
    <div className="doc-studio-shell">
      <header className="doc-topbar"><div className="doc-brand"><FileText size={22} /><span>Lumina Document Studio</span></div><div className="doc-title-area"><input className="doc-title-input" aria-label="Document title" key={selected?.id || 'untitled'} defaultValue={selected?.title || 'Untitled Document'} disabled={!selected || operationLocked} onBlur={(e) => { if (selected && e.target.value.trim() !== selected.title) documentApi.update(selected.id, { title: e.target.value.trim(), expected_version: selected.version_number }).then((updated) => { setSelected(updated); refreshDocuments(); toast.success('Renamed.'); }).catch(() => toast.error('Rename failed.')); }} /><span className="doc-save-indicator"><i className={operationLocked ? 'is-busy' : ''} />{operationLocked ? statusText : (editorHtml !== lastSavedHtmlRef.current || layoutDirty ? 'Unsaved' : 'Saved')}</span></div><div className="doc-topbar-actions"><button className="doc-btn doc-btn-import" onClick={() => importInputRef.current?.click()} disabled={operationLocked}><Upload size={18} />Import Word</button><input ref={importInputRef} type="file" className="hidden" accept=".docx" onChange={importWord} /><button className="doc-btn doc-btn-export doc-btn-pdf" onClick={() => exportDocument('pdf')} disabled={!selected || operationLocked}><Download size={18} />Export PDF</button><button className="doc-btn doc-btn-export doc-btn-word" onClick={() => exportDocument('docx')} disabled={!selected || operationLocked}><Download size={18} />Export Word</button></div></header>
      ...
      {libraryOpen && <div className="doc-library-panel"><div className="doc-library-header"><span>Document Library</span><button onClick={() => setLibraryOpen(false)}>×</button></div><div className="doc-library-list">{documents.length === 0 && <div className="doc-library-empty">No documents yet. Import a Word file or create a new document.</div>}{documents.map((doc) => <button key={doc.id} className={`doc-library-item ${selected?.id === doc.id ? 'is-selected' : ''}`} onClick={() => selectDocument(doc)} disabled={saving}><FileText size={16} /><span>{doc.title}</span></button>)}</div></div>}
      ...
      <footer className="doc-statusbar"><span>Page 1 of {pageFlow.pageCount}</span><span>{(editorHtml || '').replace(/<[^>]+>/g, ' ').trim().split(/\s+/).filter(Boolean).length} words</span><span>{normalizedPageLayout.size} · {normalizedPageLayout.orientation}</span><span>{operationLocked ? statusText : (editorHtml !== lastSavedHtmlRef.current || layoutDirty ? 'Unsaved changes' : 'Ready')}</span><div className="doc-zoom"><button onClick={() => setPage((p) => ({ ...p, zoom: Math.max(60, p.zoom - 10) }))}>−</button><span>{page.zoom}%</span><button onClick={() => setPage((p) => ({ ...p, zoom: Math.min(150, p.zoom + 10) }))}>+</button></div></footer>
    </div>
  );
}
