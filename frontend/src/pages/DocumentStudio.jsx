...
export default function DocumentStudio() {
  ...
  const studioHandoff = handoffForTarget(location.state, 'document');
  ...
  async function importWord(event) {
    const file = event.target.files?.[0]; if (!file) return; setBusy(true); setStatusText('Importing document…');
    try { const imported = await documentApi.importFile(file, { title: file.name.replace(/\.[^.]+$/, ''), category: 'Imported', tags: 'imported,document' }); selectDocument(imported, false); await refreshDocuments(); toast.success('Document imported successfully.'); }
