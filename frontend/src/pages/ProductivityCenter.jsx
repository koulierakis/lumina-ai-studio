        ...
        <Card>
          ...
          <form onSubmit={submit} className="mt-5 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <select className="lumina-input" value={form.direction} onChange={(e) => setForm({ ...form, direction: e.target.value })}>
                <option value="income">Έσοδο</option><option value="expense">Έξοδο</option>
              </select>
              <input className="lumina-input" value={form.currency} maxLength={3} onChange={(e) => setForm({ ...form, currency: e.target.value.toUpperCase() })} aria-label="Currency" />
            </div>
            <input className="lumina-input" type="number" step="0.01" min="0.01" required placeholder="Ποσό" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} />
            <input className="lumina-input" required placeholder="Κατηγορία (π.χ. Software, Commission)" value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} />
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.45fr]">
        <Card>
          ...
          <form onSubmit={save} className="mt-5 space-y-3">
            <input className="lumina-input" required placeholder="Τίτλος" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
            <input className="lumina-input" placeholder="Ερευνητικό ερώτημα" value={form.query} onChange={(e) => setForm({ ...form, query: e.target.value })} />
            <input className="lumina-input" type="url" placeholder="https://… (προαιρετικό)" value={form.source_url} onChange={(e) => setForm({ ...form, source_url: e.target.value })} />
            <textarea className="lumina-input min-h-28" placeholder="Σημειώσεις / στόχος έρευνας" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
