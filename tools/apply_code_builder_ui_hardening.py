...
def main() -> None:
    ...
    new = "  const reviewBlocked = review?.verdict === 'block';\n  const canApprove = task?.phase === 'awaiting_approval' && Boolean(preparation?.patch) && Boolean(preparation?.patch_validation) && Boolean(review) && !reviewBlocked;"
    ...
    text = replace_once(text, old_notice, new_notice, "blocked review notice")
