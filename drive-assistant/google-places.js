(()=>{
  ...
  const bypass=new WeakSet(); let loaderPromise=null;
  ...
  function loadPlaces(){if(window.google?.maps?.importLibrary)return Promise.resolve();if(loaderPromise)return loaderPromise;const key=window.__LUMINA_CONFIG__?.googleMapsApiKey;if(!key)return Promise.reject(new Error('missing-google-key'));loaderPromise=new Promise((resolve,reject)=>{const cb='__luminaGoogleMapsReady';window[cb]=()=>{delete window[cb];resolve()};const s=document.createElement('script');s.async=true;s.defer=true;s.src=`https://maps.googleapis.com/maps/api/js?key=${encodeURIComponent(key)}&v=weekly&loading=async&libraries=places&language=el&region=GR&callback=${cb}`;s.onerror=()=>reject(new Error('google-maps-load-failed'));document.head.appendChild(s)});return loaderPromise}
  ...
  async function runGoogle(item){const results=document.querySelector('#poiResults');if(results)results.innerHTML='<div class="poi-loading">Google Places: σαρώνω όλα τα κοντινά σημεία…</div>';const center=await getPos();let best=[];let bestRadius=RADII[0];for(const radius of RADII){const arr=await nearby(item.dataset.poiType,center,radius);if(arr.length>best.length){best=arr;bestRadius=radius}if(arr.length>=20)break}if(best.length){render(best,bestRadius);return true}return false}
  document.querySelector('#poiCategories')?.addEventListener('click',async e=>{const item=e.target.closest('[data-poi-type]');if(!item||bypass.has(item)){if(item)bypass.delete(item);return}if(!window.__LUMINA_CONFIG__?.googleMapsApiKey)return;e.preventDefault();e.stopImmediatePropagation();try{const ok=await runGoogle(item);if(ok)return}catch(err){console.warn('LUMINA Google Places fallback:',err)}bypass.add(item);item.click()},true);
})();