(()=>{
  ...
  function getLivePosition(){
    return new Promise((resolve,reject)=>{
      if(!navigator.geolocation){
        ...
        console.error('[LUMINA Nearby] Geolocation API unavailable',error);
        reject(error);
        return;
      }
      navigator.geolocation.getCurrentPosition(
        ...
        error=>{
          console.error('[LUMINA Nearby] GPS failed or permission denied/timed out',error);
          reject(error);
        },
  ...
  function buildOverpassQuery(type,k,v,origin){
    ...
    for(const [tagKey,tagValue] of tagsFor(type,k,v)){
      for(const element of ['node','way','relation'])clauses.push(`${element}(around:${RADIUS_M},${origin.lat},${origin.lng})[\"${tagKey}\"=\"${tagValue}\"];`);
    }
    ...
    for(const endpoint of endpoints){
      for(let attempt=1;attempt<=2;attempt++){
        try{
          ...
          lastError=error;
          console.error(`[LUMINA Nearby] Overpass failure (${endpoint}) attempt ${attempt}/2`,error);
  ...
  function areaLabel(tags){const street=[tags['addr:street'],tags['addr:housenumber']].filter(Boolean).join(' ');const area=tags['addr:suburb']||tags['addr:place']||tags['addr:city']||tags['addr:town']||tags['addr:village']||tags['is_in:city']||tags['is_in']||'';return[street,area].filter(Boolean).join(' · ')||'Κοντινή περιοχή'}
  ...
  function mapElements(data,type,k,v,origin){
    ...
    const unique=[];
    for(const item of raw){const normalizedName=norm(item.name);const duplicate=unique.some(existing=>norm(existing.name)===normalizedName&&distanceMeters(existing.point,item.point)<120);if(!duplicate)unique.push(item)}
    return unique;
  }
  ...
  async function search(type,k,v){
    ...
    try{
      ...
      renderResults(mapElements(data,type,k,v,origin));
    }catch(error){
      console.error('[LUMINA Nearby] Live search failed after retry',error);
      const msg=error?.code===1?'Χρειάζεται άδεια τοποθεσίας για να βρω κοντινά σημεία.':error?.code===3?'Το GPS άργησε να απαντήσει. Δοκιμάστε ξανά.':'Δεν ήταν δυνατή η ζωντανή αναζήτηση αυτή τη στιγμή. Δοκιμάστε ξανά.';
