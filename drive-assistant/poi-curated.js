...
async function run(k){const g=GROUPS[k],key=token();if(!g||!results)return;title.textContent=g.label.replace(/^\S+\s/,'');back.classList.remove('hidden');cats.innerHTML='';if(!key){results.innerHTML='<div class="poi-loading error">Το Mapbox δεν είναι διαθέσιμο.</div>';return}results.innerHTML='<div class="poi-loading busy"><span class="poi-spinner"></span><strong>Mapbox: βρίσκω τα κοντινότερα σημεία…</strong></div>';try{const o=await gps();const batches=await Promise.all(g.queries.map(q=>fetchQuery(q,o,key).catch(()=>[])));const items=merge(batches);if(!items.length){results.innerHTML=`<div class="poi-loading">Δεν βρέθηκαν κοντινά σημεία για ${esc(g.label)}.</div>`;return}results.innerHTML=`<div class="poi-loading"><strong>${items.length} κοντινά σημεία</strong><br><small>Mapbox · ${esc(g.label)} · από το κοντινότερο · GPS${o.accuracy?` ±${Math.round(o.accuracy)}m`:''}</small></div>`+items.map((x,i)=>`<article class="poi-result"><div><strong>${i+1}. ${esc(x.name)}</strong><span>${fd(x.distance)} · ${esc(x.area)}</span></div><button type="button" class="poi-route" data-lat="${x.point.lat}" data-lng="${x.point.lng}" data-name="${esc(x.name)}">▶ Έναρξη</button></article>`).join('')}catch(e){results.innerHTML=`<div class="poi-loading error"><strong>Mapbox: ${esc(e?.message||'σφάλμα')}</strong></div>`}}
