(()=>{
  ...
  function gps(){return new Promise((resolve,reject)=>navigator.geolocation?.getCurrentPosition(p=>resolve({lat:p.coords.latitude,lng:p.coords.longitude}),reject,{enableHighAccuracy:true,timeout:7000,maximumAge:3000})||reject(new Error('gps')))}
  ...
  function render(items){if(!results)return;if(!items.length){results.innerHTML='<div class="poi-loading">Δεν βρέθηκαν επιβεβαιωμένα σημεία κοντά στο GPS.</div>';return}results.innerHTML=`<div class="poi-loading">${items.length} επιβεβαιωμένα σημεία έως 10 km · κοντινότερα πρώτα</div>`+items.slice(0,80).map(x=>`<article class="poi-result"><div><strong>${esc(x.name)}</strong><span>${fd(x.distance)} · ${esc(x.area)}</span></div><button type="button" class="poi-route" data-lat="${x.point.lat}" data-lng="${x.point.lng}" data-name="${esc(x.name)}">▶ Έναρξη</button></article>`).join('')}
  async function run(type){if(!results)return;results.innerHTML='<div class="poi-loading">🔎 Αναζητώ πραγματικά κοντινά σημεία από πολλαπλές πηγές…</div>';try{const o=await gps();const settled=await Promise.allSettled([overpass(type,o),photon(type,o)]);const all=settled.filter(x=>x.status==='fulfilled').map(x=>x.value);render(merge(all));}catch{results.innerHTML='<div class="poi-loading">Δεν μπόρεσα να πάρω GPS ή ζωντανά δεδομένα αυτή τη στιγμή.</div>'}}
