...
async function getJSON(url,timeout=9000){const c=new AbortController(),t=setTimeout(()=>c.abort(),timeout);try{const r=await fetch(url,{cache:'no-store',signal:c.signal,headers:{Accept:'application/json'}});if(!r.ok){let detail='';try{detail=(await r.text()).slice(0,240)}catch{}throw new Error(`Mapbox HTTP ${r.status}${detail?`: ${detail}`:''}`)}return await r.json()}finally{clearTimeout(t)}}
...
function render(items,o){if(!results)return;if(!items.length){results.innerHTML='<div class="poi-loading">Το Mapbox απάντησε, αλλά δεν βρέθηκαν κοντινά σημεία αυτής της κατηγορίας.</div>';return}results.innerHTML=`<div class="poi-loading"><strong>${items.length} κοντινά σημεία</strong><br><small>Mapbox · ταξινόμηση από το κοντινότερο · GPS${o.accuracy?` ±${Math.round(o.accuracy)}m`:''}</small></div>`+items.slice(0,10).map((x,i)=>`<article class="poi-result"><div><strong>${i+1}. ${esc(x.name)}</strong><span>${fd(x.distance)} · ${esc(x.area)}</span></div><button type="button" class="poi-route" data-lat="${x.point.lat}" data-lng="${x.point.lng}" data-name="${esc(x.name)}">▶ Έναρξη</button></article>`).join('')}
async function run(type){const id=++seq,key=token();if(!key||!results)return false;results.innerHTML='<div class="poi-loading busy"><span class="poi-spinner"></span><strong>Mapbox: αναζητώ πραγματικά κοντινά σημεία…</strong></div>';try{const o=await gps();let items=[];let categoryError=null;try{items=await categorySearch(type,o,key)}catch(e){categoryError=e;console.warn('[LUMINA Mapbox category]',e)}if(!items.length){try{items=await forwardSearch(type,o,key)}catch(e){console.error('[LUMINA Mapbox forward]',e);throw categoryError||e}}if(id!==seq)return true;render(items,o);return true}catch(e){console.error('[LUMINA Mapbox POI]',e);if(id===seq)results.innerHTML='<div class="poi-loading error">Το Mapbox δεν μπόρεσε να ολοκληρώσει την αναζήτηση. Δεν εμφανίζω παλιά/εφεδρικά αποτελέσματα.</div>';return true}}
