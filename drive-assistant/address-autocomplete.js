(() => {
  ...
  function fromNominatim(item){const a=item.address||{},road=a.road||a.pedestrian||'',city=locality(a),name=road||a.city||a.town||a.village||item.name||item.display_name?.split(',')[0]||'';return{lat:+item.lat,lng:+item.lon,road,name,city,countrycode:'gr',display:item.display_name||[name,city].filter(Boolean).join(', '),kind:road?'road':'place'};}
  function fromPhoton(feature){const p=feature.properties||{},c=feature.geometry?.coordinates||[],road=p.street||((['residential','primary','secondary','tertiary'].includes(p.osm_value))?p.name:'')||'',city=p.city||p.town||p.village||p.district||p.county||p.state||'',name=road||p.name||city||'';return{lat:+c[1],lng:+c[0],road,name,city,countrycode:String(p.countrycode||'').toLowerCase(),display:[name,city,p.state].filter(Boolean).join(', '),kind:road?'road':'place'};}
  const searchable=x=>norm([x.road,x.name,x.city,x.display].filter(Boolean).join(' '));
  const matches=(x,q)=>searchable(x).includes(norm(q));
  const score=(x,q)=>{const nq=norm(q),r=norm(x.road||x.name),c=norm(x.city);if(r.startsWith(nq))return 0;if(r.includes(nq))return 1;if(c.startsWith(nq))return 2;if(c.includes(nq))return 3;return 9;};

  function render(items,q){
    const seen=new Set();
    results=items.filter(x=>x.countrycode==='gr'&&Number.isFinite(x.lat)&&Number.isFinite(x.lng)&&matches(x,q)).filter(x=>{const key=`${norm(x.road||x.name)}|${norm(x.city)}|${x.lat.toFixed(4)}|${x.lng.toFixed(4)}`;if(seen.has(key))return false;seen.add(key);return true;}).sort((a,b)=>score(a,q)-score(b,q)||norm(a.road||a.name).localeCompare(norm(b.road||b.name),'el')||norm(a.city).localeCompare(norm(b.city),'el')).slice(0,14);
    active=-1;
  ...
  async function photon(q,signal){const u=new URL('https://photon.komoot.io/api/');u.searchParams.set('q',q);u.searchParams.set('lang','el');u.searchParams.set('limit','30');const r=await fetch(u,{signal,headers:{Accept:'application/json'}});if(!r.ok)throw new Error('photon');const j=await r.json();return(j.features||[]).map(fromPhoton).filter(x=>x.countrycode==='gr'&&x.display);}
  async function nominatim(q,signal){const p=new URLSearchParams({format:'jsonv2',addressdetails:'1',countrycodes:'gr',limit:'30',dedupe:'0','accept-language':'el',q});const r=await fetch(`https://nominatim.openstreetmap.org/search?${p}`,{signal,headers:{Accept:'application/json'}});if(!r.ok)throw new Error('nominatim');const j=await r.json();return(Array.isArray(j)?j:[]).map(fromNominatim);}
  async function query(value){const q=value.trim();if(q.length<3||!navigator.onLine)return hide();if(controller)controller.abort();controller=new AbortController();try{const settled=await Promise.allSettled([photon(q,controller.signal),nominatim(q,controller.signal)]),merged=settled.flatMap(x=>x.status==='fulfilled'?x.value:[]);render(merged,q);}catch(e){if(e.name!=='AbortError')hide();}}
  input.addEventListener('input',()=>{delete input.dataset.selectedLat;delete input.dataset.selectedLng;delete input.dataset.selectedName;clearTimeout(timer);const v=input.value;if(v.trim().length<3)return hide();timer=setTimeout(()=>query(v),350);});
