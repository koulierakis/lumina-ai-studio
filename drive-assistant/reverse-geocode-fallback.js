(()=>{
  ...
  async function nominatim(lat,lng){
    ...
    const r=await fetch(u,{headers:{Accept:'application/json'},cache:'no-store'});if(!r.ok)throw new Error('nominatim');
  ...
  function tick(){
    ...
    if(p&&Number.isFinite(p.lat)&&Number.isFinite(p.lng))update(p.lat,p.lng);
  }
