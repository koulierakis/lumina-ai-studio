...
(() => {
  ...
  function loadEnhancedPoi() {
    ...
    script.src = './poi-enhanced.js?v=43';
    script.async = true;
  ...
  function buildHomeToolbar(){
    ...
    bar.id='homeToolbar';
    bar.className='home-toolbar pretrip-ui';
    bar.setAttribute('aria-label','Κύρια εργαλεία Drive');
    const drive=travel.querySelector('[data-travel-mode="drive"]');
    const walk=travel.querySelector('[data-travel-mode="walk"]');
    const poi=$('#poiOpenBtn');
    ...
    travel.classList.add('toolbar-source-hidden');
    actions.classList.add('toolbar-source-hidden');
  }
  ...
  function bind(){
    ...
    $('#navStopBtn')?.addEventListener('click',()=>{if(freeDriveIsOn())safeClick(free);setTimeout(leaveFreeDriveUI,0)});
    const statusBtn=document.createElement('button');statusBtn.type='button';statusBtn.id='navStatusBtn';statusBtn.className='nav-mini';statusBtn.textContent='🔊';statusBtn.setAttribute('aria-label','Κατάσταση οδήγησης');statusBtn.addEventListener('click',speakStatus);const dock=$('#navVoiceDock');if(dock&&!$('#navStatusBtn'))dock.appendChild(statusBtn);
    const mic=$('#navVoiceBtn');let timer=null;mic?.addEventListener('pointerdown',()=>{timer=setTimeout(reportDanger,900)});['pointerup','pointercancel','pointerleave'].forEach(ev=>mic?.addEventListener(ev,()=>{if(timer)clearTimeout(timer);timer=null}));
    syncFreeDrive();observeArrival();
  }
